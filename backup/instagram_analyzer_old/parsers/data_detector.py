...
class DataDetector:
    ...
    def _categorize_file(self, file_path: Path, structure: dict[str, Any]) -> None:
        ...
        try:
            ...
            print(
                f"DEBUG: Processing file: {file_path.name} -> Relative parts: {path_parts}"
            )
        ...
        if path_parts == ["personal_information", "personal_information.json"]:
            print(f"DEBUG: Categorized {filename} as PROFILE")
        ...
        if path_parts == ["your_instagram_activity", "media", "posts_1.json"]:
            if self._is_posts_file(file_path):
                print(f"DEBUG: Categorized {filename} as POSTS")
            ...
            if self._is_stories_file(file_path):
                print(f"DEBUG: Categorized {filename} as STORIES")
            ...
            if self._is_reels_file(file_path):
                print(f"DEBUG: Categorized {filename} as REELS")
                structure["reel_files"].append(file_path)
                return

        # --- Archived and Deleted Content ---
        if path_parts == ["your_instagram_activity", "content", "archived_posts.json"]:
            print(f"DEBUG: Categorized {filename} as ARCHIVED_POSTS")
            ...
            print(f"DEBUG: Categorized {filename} as RECENTLY_DELETED")
        ...
        if path_parts[:-1] == ["your_instagram_activity", "story_interactions"]:
            ...
            print(
                f"DEBUG: Categorized {filename} as STORY_INTERACTION ({interaction_type})"
            )
        ...
        if path_parts == ["your_instagram_activity", "likes", "liked_posts.json"]:
            if self._is_engagement_file(file_path, "liked_posts"):
                print(f"DEBUG: Categorized {filename} as LIKED_POSTS")
            ...
            if self._is_engagement_file(file_path, "post_comments"):
                print(f"DEBUG: Categorized {filename} as POST_COMMENTS")
            ...
            if self._is_engagement_file(file_path, "reel_comments"):
                print(f"DEBUG: Categorized {filename} as REEL_COMMENTS")
        ...
        try:
            ...
            if engagement_type == "liked_posts":
                ...
                return (
                    ...
                    and data  # check if not empty
                    and "string_map_data" in data[0]
