...
class TestEngagementParserWrapper(EngagementParser):
    """Wrapper around EngagementParser to expose protected methods for testing."""

    def parse_liked_posts(self, file_path: Path) -> list[dict[str, Any]]:
        """Expose protected method for testing."""
        return self._parse_liked_posts(file_path)

    def parse_post_comments(self, file_path: Path) -> list[dict[str, Any]]:
        """Expose protected method for testing."""
        return self._parse_post_comments(file_path)

    def parse_reel_comments(self, file_path: Path) -> list[dict[str, Any]]:
        """Expose protected method for testing."""
        return self._parse_reel_comments(file_path)
...
class TestEngagementParser:
    ...
    def setup_method(self):
        ...
        self.parser = TestEngagementParserWrapper()
    ...
    def test_parse_liked_posts_basic(self, tmp_path):
        ...
        # Parse
        liked_posts = self.parser.parse_liked_posts(liked_posts_file)

        assert len(liked_posts) == 1
        assert liked_posts[0]["title"] == "Test Post"
        assert liked_posts[0]["href"] == "https://www.instagram.com/p/test123/"
        assert liked_posts[0]["timestamp"] == 1672574400
    ...
    def test_parse_post_comments_basic(self, tmp_path):
        ...
        # Parse
        comments = self.parser.parse_post_comments(comments_file)

        assert len(comments) == 1
        assert comments[0]["post_url"] == "https://www.instagram.com/p/test123/"
        assert comments[0]["text"] == "Test comment text"
        assert comments[0]["timestamp"] == 1672574400
    ...
    def test_parse_reel_comments_basic(self, tmp_path):
        ...
        # Parse
        reel_comments = self.parser.parse_reel_comments(reel_comments_file)

        assert len(reel_comments) == 1
        assert reel_comments[0]["title"] == "Test Reel Comment"
        assert reel_comments[0]["href"] == "https://www.instagram.com/reel/test123/"
        assert reel_comments[0]["text"] == "Great reel!"
        assert reel_comments[0]["timestamp"] == 1672574400
    ...
    def test_parse_from_nonexistent_file(self):
        ...
        liked_posts = self.parser.parse_liked_posts(nonexistent)
        assert liked_posts == []

        comments = self.parser.parse_post_comments(nonexistent)
        assert comments == []

        reel_comments = self.parser.parse_reel_comments(nonexistent)
        assert reel_comments == []
    ...
    def test_parse_from_invalid_json(self, tmp_path):
        ...
        liked_posts = self.parser.parse_liked_posts(invalid_file)
        assert liked_posts == []

        comments = self.parser.parse_post_comments(invalid_file)
        assert comments == []

        reel_comments = self.parser.parse_reel_comments(invalid_file)
        assert reel_comments == []
    ...
    def test_parse_multiple_entries(self, tmp_path):
        ...
        # Verify posts are correctly parsed
        titles = [post["title"] for post in liked_posts]
        assert "Post 1" in titles
        assert "Post 2" in titles
        hrefs = [post["href"] for post in liked_posts]
        assert "https://www.instagram.com/p/test123/" in hrefs
        assert "https://www.instagram.com/p/test456/" in hrefs
    ...
    def test_error_handling_graceful(self, tmp_path):
        ...
        # Should handle gracefully
        liked_posts = self.parser.parse_liked_posts(malformed_file)
        assert isinstance(liked_posts, list)

        comments = self.parser.parse_post_comments(malformed_file)
        assert isinstance(comments, list)

        reel_comments = self.parser.parse_reel_comments(malformed_file)
