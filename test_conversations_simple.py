        ...
        if conversation:
            ...
            if conversation.messages:
                ...
                for msg in conversation.messages[-3:]:
                    sender = msg.sender_name[:20] + "..." if len(msg.sender_name) > 20 else msg.sender_name
                    content = msg.content[:50] + "..." if msg.content and len(msg.content) > 50 else msg.content or "[Sin texto]"
...
def test_multiple_conversations():
    ...
    try:
        ...
        direct_convs = len([c for c in conversations if c.conversation_type == ConversationType.DIRECT])
        group_convs = len([c for c in conversations if c.conversation_type == ConversationType.GROUP])
    ...
    try:
        ...
        if analysis.most_frequent_contacts:
            ...
            for i, contact in enumerate(analysis.most_frequent_contacts[:3], 1):
                name = contact['name'][:25] + "..." if len(contact['name']) > 25 else contact['name']
