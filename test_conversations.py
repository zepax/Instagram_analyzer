...
def test_conversation_analysis():
    ...
    if analysis.most_frequent_contacts:
        ...
        for i, contact in enumerate(analysis.most_frequent_contacts[:5], 1):
            print(f"   {i}. {contact['name'][:30]} - {contact['total_messages']} mensajes")
    ...
    if analysis.thread_analysis:
        ...
        print(f"   • Promedio mensajes/hilo: {thread_info.get('avg_thread_length', 0):.1f}")
        print(f"   • Duración promedio: {thread_info.get('avg_thread_duration_minutes', 0):.1f} minutos")
    
    # Tópicos populares
    if analysis.popular_topics:
        print(f"\n🏷️ Tópicos más populares:")
        for i, topic in enumerate(analysis.popular_topics[:5], 1):
            print(f"   {i}. {topic['topic']} (en {topic['conversations']} conversaciones)")
    
    # Tiempos de respuesta
    if analysis.response_time_analysis:
        resp_analysis = analysis.response_time_analysis
        print(f"\n⏱️ Análisis de tiempos de respuesta:")
        print(f"   • Tiempo promedio: {resp_analysis.get('avg_response_time_minutes', 0):.1f} minutos")
        print(f"   • Respuestas rápidas (<5min): {resp_analysis.get('fast_response_percentage', 0):.1f}%")
        print(f"   • Respuestas lentas (>1h): {resp_analysis.get('slow_response_percentage', 0):.1f}%")
...
def test_single_conversation():
    ...
    conv_file = Path("examples/instagram-florenescobar-2025-07-13-pcFuHXmB/your_instagram_activity/messages/inbox/paolacastillo_513456650044931/message_1.json")
    
    if not conv_file.exists():
        print(f"❌ Archivo no encontrado: {conv_file}")
        # Intentar encontrar cualquier archivo de conversación
        inbox_dir = Path("examples/instagram-florenescobar-2025-07-13-pcFuHXmB/your_instagram_activity/messages/inbox")
        if inbox_dir.exists():
    ...
    if conversation.metrics:
        ...
        print(f"   • Hora más activa: {metrics.most_active_hour}:00" if metrics.most_active_hour else "")
    ...
    for msg in recent_messages:
        timestamp = msg.timestamp.strftime('%Y-%m-%d %H:%M') if msg.timestamp else 'Sin fecha'
        content = msg.content[:50] + "..." if msg.content and len(msg.content) > 50 else msg.content or "[Sin contenido]"
    ...
    if conversation.threads:
        ...
        for i, thread in enumerate(conversation.threads[:3], 1):
            duration = f" ({thread.duration_minutes:.0f}min)" if thread.duration_minutes else ""
