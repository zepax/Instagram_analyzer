...
from ..utils import get_activity_days_of_week, get_activity_hours
...
class TemporalAnalyzer:
    ...
    def analyze(
        self, posts: list[Post], stories: list[Story], reels: list[Reel]
    ) -> dict[str, Any]:
    ...
    def _analyze_hourly_patterns(self, content: list) -> dict[str, Any]:
        ...
        quiet_hour = (
            min(hourly_activity.items(), key=lambda x: x[1])
            if hourly_activity
            else (0, 0)
        )
        ...
        return {
            ...
            "morning_activity_rate": (
                (morning_posts / total_posts * 100) if total_posts else 0
            ),
            "afternoon_activity_rate": (
                (afternoon_posts / total_posts * 100) if total_posts else 0
            ),
            "evening_activity_rate": (
                (evening_posts / total_posts * 100) if total_posts else 0
            ),
            "night_activity_rate": (
                (night_posts / total_posts * 100) if total_posts else 0
            ),
            "most_active_period": self._get_most_active_period(
                morning_posts, afternoon_posts, evening_posts, night_posts
            ),
        }

    def _get_most_active_period(
        self, morning: int, afternoon: int, evening: int, night: int
    ) -> str:
    ...
    def _analyze_daily_patterns(self, content: list) -> dict[str, Any]:
        ...
        # Find peak days
        peak_day = (
            max(daily_activity.items(), key=lambda x: x[1]) if daily_activity else ("", 0)
        )
        quiet_day = (
            min(daily_activity.items(), key=lambda x: x[1]) if daily_activity else ("", 0)
        )

        # Weekend vs weekday analysis
        weekend_posts = daily_activity.get("Saturday", 0) + daily_activity.get(
            "Sunday", 0
        )
        weekday_posts = sum(
            daily_activity.get(day, 0)
        ...
        return {
            ...
            "weekend_activity_rate": (
                (weekend_posts / total_posts * 100) if total_posts else 0
            ),
            "weekday_activity_rate": (
                (weekday_posts / total_posts * 100) if total_posts else 0
            ),
            "weekend_vs_weekday_ratio": (
                (weekend_posts / weekday_posts) if weekday_posts else 0
            ),
        }
    ...
    def _analyze_monthly_trends(self, content: list) -> dict[str, Any]:
        ...
        most_active_month = (
            max(monthly_counts.items(), key=lambda x: x[1]) if monthly_counts else ("", 0)
        )
        least_active_month = (
            min(monthly_counts.items(), key=lambda x: x[1]) if monthly_counts else ("", 0)
        )
    ...
    def _analyze_activity_streaks(self, content: list) -> dict[str, Any]:
        ...
        return {
            ...
            "average_posts_per_active_day": (
                sum(daily_posts.values()) / len(daily_posts) if daily_posts else 0
            ),
    ...
    def _find_peak_periods(self, content: list) -> dict[str, Any]:
        ...
        # Find peak week
        peak_week = (
            max(weekly_counts.items(), key=lambda x: x[1]) if weekly_counts else ("", 0)
        )

        # Calculate activity intensity
        total_weeks = len(weekly_counts)
        average_weekly_posts = (
            sum(weekly_counts.values()) / total_weeks if total_weeks else 0
        )

        # Find weeks above average
        above_average_weeks = sum(
            1 for count in weekly_counts.values() if count > average_weekly_posts
        )
        ...
        return {
            ...
            "activity_intensity_score": (
                (above_average_weeks / total_weeks * 100) if total_weeks else 0
            ),
        }
