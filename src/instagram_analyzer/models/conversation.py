...
class MessageMedia(BaseModel):
    ...
    creation_timestamp: Optional[datetime] = Field(
        None, description="When media was created"
    )
...
class ShareContent(BaseModel):
    ...
    original_content_owner: Optional[str] = Field(
        None, description="Original content creator"
    )
...
class Message(BaseModel):
    ...
    # Media attachments
    photos: list[MessageMedia] = Field(
        default_factory=list, description="Photo attachments"
    )
    audio_files: list[MessageMedia] = Field(
        default_factory=list, description="Audio attachments"
    )
    videos: list[MessageMedia] = Field(
        default_factory=list, description="Video attachments"
    )
    gifs: list[MessageMedia] = Field(default_factory=list, description="GIF attachments")
    stickers: list[MessageMedia] = Field(
        default_factory=list, description="Sticker attachments"
    )

    # Interactions
    reactions: list[MessageReaction] = Field(
        default_factory=list, description="Reactions to this message"
    )
    share: Optional[ShareContent] = Field(None, description="Shared content information")

    # Metadata
    is_geoblocked_for_viewer: bool = Field(
        False, description="Whether message is geoblocked"
    )
    is_unsent_image_by_messenger_kid_parent: bool = Field(
        False, description="Messenger Kids flag"
    )

    # Derived fields
    timestamp: Optional[datetime] = Field(
        None, description="Parsed timestamp as datetime"
    )
...
class Participant(BaseModel):
    ...
    join_timestamp: Optional[datetime] = Field(
        None, description="When they joined the conversation"
    )
    leave_timestamp: Optional[datetime] = Field(
        None, description="When they left the conversation"
    )
...
class ConversationThread(BaseModel):
    ...
    root_message_id: Optional[str] = Field(
        None, description="ID of the message that started the thread"
    )
    messages: list[Message] = Field(
        default_factory=list, description="Messages in this thread"
    )
    participants: list[str] = Field(
        default_factory=list, description="Participants in this thread"
    )
    ...
    duration_minutes: Optional[float] = Field(
        None, description="Thread duration in minutes"
    )
...
class Conversation(BaseModel):
    ...
    conversation_type: ConversationType = Field(
        ConversationType.DIRECT, description="Type of conversation"
    )
    ...
    is_still_participant: bool = Field(
        True, description="Whether account owner is still in conversation"
    )
    ...
    threads: list[ConversationThread] = Field(
        default_factory=list, description="Message threads"
    )

    # Conversation features
    magic_words: list[str] = Field(
        default_factory=list, description="Magic words/triggers"
    )
    joinable_mode: Optional[dict[str, Any]] = Field(
        None, description="Group join settings"
    )

    # Analytics
    metrics: Optional[ConversationMetrics] = Field(
        None, description="Calculated conversation metrics"
    )

    # Temporal analysis
    activity_patterns: dict[str, Any] = Field(
        default_factory=dict, description="Activity pattern analysis"
    )
    peak_activity_periods: list[dict[str, Any]] = Field(
        default_factory=list, description="Periods of high activity"
    )

    # Content analysis
    dominant_message_types: list[str] = Field(
        default_factory=list, description="Most common message types"
    )
    keyword_frequency: dict[str, int] = Field(
        default_factory=dict, description="Frequency of keywords"
    )
    sentiment_scores: dict[str, float] = Field(
        default_factory=dict, description="Sentiment analysis by participant"
    )

    # Privacy and security
    contains_sensitive_data: bool = Field(
        False, description="Whether conversation contains sensitive information"
    )
    anonymization_applied: bool = Field(
        False, description="Whether data has been anonymized"
    )

    # Raw data
    raw_data: dict[str, Any] = Field(
        default_factory=dict, description="Original conversation data"
    )
...
class ConversationAnalysis(BaseModel):
    ...
    conversation_sizes: dict[int, int] = Field(
