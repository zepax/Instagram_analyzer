...
class Comment(BaseModel):
    ...
    is_reply: bool = Field(
        False, description="Whether this is a reply to another comment"
    )
...
class StoryInteraction(BaseModel):
    """Represents a single interaction with a story."""

    interaction_type: str = Field(
        ..., description="Type of interaction (e.g., 'poll', 'quiz')"
    )
    title: str = Field(
        ..., description="Title of the story interaction (e.g., poll question)"
    )
    timestamp: datetime = Field(..., description="When the interaction occurred")
    response: str = Field(..., description="User's response to the interaction")

    # Raw data
    raw_data: dict[str, Any] = Field(
        default_factory=dict, description="Raw interaction data"
    )
