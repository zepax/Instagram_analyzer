...
from ..models.conversation import (
    Message, Conversation, ConversationThread, MessageType,
    ConversationAnalysis, ConversationMetrics
)
...
class ThreadReconstructionEngine:
        ...
        for message in messages:
            if (current_thread is None or 
                (message.timestamp and current_thread.end_time and
                 message.timestamp - current_thread.end_time > self.time_gap_threshold)):
    ...
    def _topic_based_threading(self, messages: List[Message]) -> List[ConversationThread]:
        ...
        for i, message in enumerate(messages):
            ...
            if current_thread and current_topics:
                similarity = len(msg_topics.intersection(current_topics)) / len(msg_topics.union(current_topics))
            else:
                similarity = 0
    ...
    def _interaction_based_threading(self, messages: List[Message]) -> List[ConversationThread]:
        ...
        for chain in reply_chains:
            if len(chain) >= self.min_thread_messages:
                thread = ConversationThread(
                    ...
                    messages=chain,
                    participants=list(set(msg.sender_name for msg in chain)),
        ...
        for group in reaction_groups:
            if len(group) >= self.min_thread_messages:
                thread = ConversationThread(
                    ...
                    messages=group,
                    participants=list(set(msg.sender_name for msg in group)),
    ...
    def _extract_message_topics(self, message: Message) -> Set[str]:
        ...
        if message.content:
            ...
            # Filter common words
            stop_words = {'para', 'como', 'esta', 'pero', 'todo', 'muy', 'que', 'con', 'una', 'por', 'más', 
                         'hola', 'the', 'and', 'you', 'for', 'are', 'not', 'this', 'that', 'jaja', 'jajaja'}
    ...
    def _detect_reply_chains(self, messages: List[Message]) -> List[List[Message]]:
        ...
        for i in range(len(messages) - 1):
            ...
            for j in range(i + 1, min(i + 10, len(messages))):  # Look ahead max 10 messages
                next_msg = messages[j]
                
                # Check if it's a potential reply
                if (next_msg.timestamp and current_msg.timestamp and
                    (next_msg.timestamp - current_msg.timestamp).total_seconds() < 300 and  # 5 minutes
                    next_msg.sender_name != last_sender):
            ...
            if msg.reactions:
                ...
                for j in range(start_idx, end_idx):
                    if j != i:
                        ...
                        if (other_msg.timestamp and msg.timestamp and
                            abs((other_msg.timestamp - msg.timestamp).total_seconds()) < 600):  # 10 minutes
    ...
    def _merge_and_optimize_threads(self, *thread_lists) -> List[ConversationThread]:
        ...
        for thread in sorted_threads:
            thread_message_ids = {msg.message_id for msg in thread.messages if msg.message_id}
    ...
    def _post_process_threads(self, threads: List[ConversationThread]) -> List[ConversationThread]:
    ...
    def _infer_thread_topic(self, messages: List[Message]) -> str:
        ...
        return ', '.join(list(content_topics)[:3]) if content_topics else 'General conversation'
...
class ConversationAnalyzer:
    ...
    def load_conversations(self, conversations_dir: Optional[Path] = None) -> List[Conversation]:
        ...
        if conversations_dir is None:
            conversations_dir = self.data_root / 'your_instagram_activity' / 'messages' / 'inbox'
        ...
        for conversation in self.conversations:
            conversation.threads = self.thread_engine.reconstruct_threads(conversation.messages)
    ...
    def _enhance_analysis_with_advanced_metrics(self, analysis: ConversationAnalysis) -> ConversationAnalysis:
    ...
    def _calculate_response_times(self) -> Dict[str, float]:
        ...
        for conv in self.conversations:
            ...
            for i in range(1, len(conv.messages)):
                ...
                if (current_msg.sender_name != prev_msg.sender_name and
                    current_msg.timestamp and prev_msg.timestamp):
        ...
        return {
            ...
            'fast_response_percentage': len([t for t in all_response_times if t <= 5]) / len(all_response_times) * 100,
            'slow_response_percentage': len([t for t in all_response_times if t >= 60]) / len(all_response_times) * 100
        }
    
    def _analyze_conversation_lengths(self) -> Dict[str, int]:
        """Analyze distribution of conversation lengths."""
        lengths = [conv.metrics.total_messages if conv.metrics else 0 for conv in self.conversations]
    ...
    def _analyze_threads(self) -> Dict[str, Any]:
        ...
        thread_durations = [thread.duration_minutes for thread in all_threads if thread.duration_minutes]
        
        analysis = {
            'total_threads': len(all_threads),
            'avg_thread_length': statistics.mean(thread_lengths) if thread_lengths else 0,
            'avg_thread_duration_minutes': statistics.mean(thread_durations) if thread_durations else 0,
            'threads_per_conversation': len(all_threads) / len(self.conversations) if self.conversations else 0
        }
    ...
    def _analyze_popular_topics(self) -> List[Dict[str, Any]]:
        ...
        for topic, frequency in topic_frequency.most_common(20):
            popular_topics.append({
                'topic': topic,
                'frequency': frequency,
                'conversations': len([c for c in self.conversations 
    ...
    def search_conversations(self, query: str, search_content: bool = True, 
                           search_participants: bool = True) -> List[Conversation]:
    ...
    def export_conversation_summary(self, output_path: Path) -> Path:
        ...
        summary_data = self.analysis.dict()
