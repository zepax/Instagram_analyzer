...
from ..models.conversation import (
    Message, MessageType, MessageReaction, MessageMedia, ShareContent,
    Conversation, ConversationType, Participant, ConversationMetrics,
    ConversationThread, ConversationAnalysis
)
...
class ConversationParser:
    ...
    def _parse_conversation_data(self, data: Dict[str, Any], file_path: Path) -> Conversation:
        ...
        conv_type = ConversationType.GROUP if len(participants) > 2 else ConversationType.DIRECT
    ...
    def _parse_participants(self, participants_data: List[Dict[str, Any]]) -> List[Participant]:
    ...
    def _parse_messages(self, messages_data: List[Dict[str, Any]], conversation_id: str) -> List[Message]:
    ...
    def _parse_single_message(self, msg_data: Dict[str, Any], conversation_id: str) -> Optional[Message]:
        ...
        audio_files = self._parse_media_list(msg_data.get('audio_files', []), MessageType.AUDIO)
        ...
        stickers = self._parse_media_list(msg_data.get('stickers', []), MessageType.STICKER)
        ...
        message = Message(
            ...
            is_unsent_image_by_messenger_kid_parent=msg_data.get('is_unsent_image_by_messenger_kid_parent', False),
    ...
    def _parse_reactions(self, reactions_data: List[Dict[str, Any]]) -> List[MessageReaction]:
    ...
    def _parse_media_list(self, media_data: List[Dict[str, Any]], media_type: MessageType) -> List[MessageMedia]:
    ...
    def _parse_share_content(self, share_data: Optional[Dict[str, Any]]) -> Optional[ShareContent]:
    ...
    def _calculate_conversation_metrics(self, messages: List[Message], participants: List[Participant]) -> ConversationMetrics:
        ...
        most_active_participant = message_count_by_participant.most_common(1)[0][0] if message_count_by_participant else None
    ...
    def _generate_threads(self, messages: List[Message]) -> List[ConversationThread]:
        ...
        for i, message in enumerate(messages):
            ...
            start_new_thread = (
                current_thread is None or
                (message.timestamp and current_thread.end_time and 
                 (message.timestamp - current_thread.end_time).total_seconds() > thread_gap_minutes * 60)
            )
    ...
    def _analyze_activity_patterns(self, messages: List[Message]) -> Dict[str, Any]:
        ...
        # Daily distribution
        daily_counts = Counter(msg.timestamp.strftime('%A') for msg in messages if msg.timestamp)
        
        # Monthly distribution
        monthly_counts = Counter(msg.timestamp.strftime('%Y-%m') for msg in messages if msg.timestamp)
        ...
        for msg in messages:
            if msg.content:
                ...
                filtered_words = [
                    ...
                    if len(word) > 3 and word not in {'para', 'como', 'esta', 'pero', 'todo', 'muy', 'que', 'con', 'una', 'por', 'más', 'hola', 'the', 'and', 'you', 'for', 'are', 'not', 'this', 'that'}
                ]
    ...
    def generate_conversation_analysis(self, conversations: Optional[List[Conversation]] = None) -> ConversationAnalysis:
        ...
        total_messages = sum(conv.metrics.total_messages if conv.metrics else 0 for conv in conversations)
        ...
        most_active = sorted(
            [{'title': conv.title, 'messages': conv.metrics.total_messages if conv.metrics else 0, 'id': conv.conversation_id} 
             for conv in conversations],
        ...
        for conv in conversations:
            if conv.conversation_type == ConversationType.DIRECT:
                ...
                if other_participants:
                    contact_frequency[other_participants[0]] += conv.metrics.total_messages if conv.metrics else 0
        ...
        group_count = len([c for c in conversations if c.conversation_type == ConversationType.GROUP])
        direct_count = len([c for c in conversations if c.conversation_type == ConversationType.DIRECT])
        
        group_vs_direct_ratio = {
            'group_percentage': (group_count / total_conversations * 100) if total_conversations > 0 else 0,
            'direct_percentage': (direct_count / total_conversations * 100) if total_conversations > 0 else 0
        }
        ...
        # Privacy analysis
        conversations_with_sensitive = len([c for c in conversations if c.contains_sensitive_data])
