...
@click.option("--include-media", is_flag=True, 
              help="Include media analysis (slower)")
@click.option("--anonymize", is_flag=True,
              help="Anonymize sensitive data in reports")
@click.pass_context
def analyze(ctx: click.Context, data_path: Path, output: Optional[Path], 
           format: str, include_media: bool, anonymize: bool) -> None:
