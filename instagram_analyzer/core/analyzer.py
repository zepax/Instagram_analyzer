...
class InstagramAnalyzer:
    ...
    def __init__(self, data_path: Path, lazy_loading: bool = True):
    ...
    def _load_stories_lazy(self) -> list[Story]:
        ...
        for file_path in files:
            try:
                # Use streaming parser for memory efficiency
                stories.extend(self.parser.parse_stories_from_file(str(file_path)))
            except Exception as e:
    ...
    def _load_archived_posts_lazy(self) -> list[Post]:
        ...
        for file_path in files:
            try:
                archived_posts.extend(self.parser.parse_posts_from_file(str(file_path)))
            except Exception as e:
                self.logger.warning(
