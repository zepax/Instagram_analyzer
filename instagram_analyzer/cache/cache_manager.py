...
import threading
import time
from typing import Any, Dict, Optional, Set, List, Union, Callable
import hashlib
import json
...
class CacheManager:
    ...
    def set(
        ...
        with self._lock:
            ...
            try:
                ...
                if (self.memory_cache and 
                    self.config.memory_cache_enabled and 
                    not disk_only):
                ...
                # Store in disk cache
                if (self.disk_cache and 
                    self.config.disk_cache_enabled and 
                    not memory_only):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if total_requests > 0:
            ...
            stats["global"]["memory_hit_rate"] = self._global_stats["memory_hits"] / total_requests
            stats["global"]["disk_hit_rate"] = self._global_stats["disk_hits"] / total_requests
        else:
                ...
                try:
                    ...
                    if self._warming_queue:
                        ...
                        for key in keys_to_warm:
                            ...
                            if (self.disk_cache and self.memory_cache and
                                self.disk_cache.exists(key) and
                                not self.memory_cache.exists(key)):
