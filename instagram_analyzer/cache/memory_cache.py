...
from typing import Any, Dict, Optional, Set, Callable, TypeVar, Generic
import sys
import gc
...
class MemoryCache(Generic[T]):
    ...
    def set(
        ...
        if size_bytes > self.config.memory_limit:
            logger.warning(f"Object too large for cache: {size_bytes} > {self.config.memory_limit}")
            return False
    ...
    def _evict_if_needed(self, new_size: int) -> None:
        ...
        while (self._current_size + new_size > self.config.memory_limit and 
               len(self._data) > 0):
            ...
            if self.config.eviction_policy == "lru":
                ...
                key = min(self._data.keys(), 
                         key=lambda k: self._data[k].access_count)
            else:  # FIFO
                # Remove oldest entry
                key = min(self._data.keys(),
