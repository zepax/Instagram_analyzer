    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            cache_key = _generate_function_cache_key(
                ...
                include_args, include_kwargs, exclude_set
            )
            ...
            try:
                ...
                should_cache = (
                    (result is not None or cache_none) and
                    result is not ...  # Don't cache Ellipsis
                )
                ...
                if cache_exceptions:
                    try:
                        ...
                        cache_mgr.set(
                            cache_key, exception_data, ttl=ttl,
                            memory_only=memory_only,
                            disk_only=disk_only
                        )
                        logger.debug(f"Cached exception for function {func.__name__}")
                    except Exception as cache_error:
                        logger.error(f"Error caching exception for {func.__name__}: {cache_error}")
        ...
        wrapper.cache_invalidate = lambda *args, **kwargs: _invalidate_function_cache(
            func, cache_mgr, args, kwargs, key_prefix, 
            include_args, include_kwargs, exclude_set
        )
...
def cached_property(
    ...
    **kwargs
    ...
    def decorator(func: Callable) -> property:
        cached_func = cached(
            ttl=ttl,
            key_prefix=f"property_{func.__name__}",
            **kwargs
        )(func)
...
def _get_function_cache_info(
    ...
    matching_keys = [key for key in all_keys if cache_manager._key_matches_pattern(key, pattern)]
