                ...
                with self._db_lock:
                    ...
                    try:
                        conn.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                        """, (
                            key, filename, current_time, current_time, effective_ttl,
                            len(final_data), compressed, compression_ratio
                        ))
                        conn.commit()
                    finally:
                        conn.close()
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                **self._stats,
                "hit_rate": hit_rate,
                "disk_usage_pct": (self._stats["size_bytes"] / self.config.disk_limit) * 100,
                "avg_entry_size": self._stats["size_bytes"] / max(1, self._stats["entry_count"]),
