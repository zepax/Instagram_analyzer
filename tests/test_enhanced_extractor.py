...
from instagram_analyzer.extractors.conversation_extractor import (
    ...
    quick_extract_top_conversations,
)
...
def test_enhanced_extractor():
    ...
    criteria_conversations = extractor.extract_conversations_by_criteria(
        date_range=(start_date, end_date), min_message_count=10
    )
    print(
        f"✅ Found {len(criteria_conversations)} conversations from last year with 10+ messages"
    )
    ...
    conversations, analysis = extractor.extract_and_analyze(
        export_path=export_path, include_advanced_analytics=True, anonymize=False
    )
...
def test_conversation_iterator():
    ...
    for batch in iterator:
        ...
        print(
            f"   Batch {batch_count}: {batch_conversations} conversations, {batch_messages} messages"
        )
...
def test_keyword_extraction():
    ...
    for i, conv in enumerate(matching_conversations[:5], 1):
        ...
        for keyword in keywords:
            if any(
                msg.content and keyword.lower() in msg.content.lower()
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"   • Processing speed: {final_stats.get('processing_rate_conversations_per_second', 0):.1f} conv/sec"
        )
        print(
            f"   • Average messages per conversation: {final_stats.get('avg_messages_per_conversation', 0):.1f}"
        )
